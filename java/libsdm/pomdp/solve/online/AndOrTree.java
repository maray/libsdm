...
public class AndOrTree {
    ...
    protected BeliefValueFunction offlineLower;
    protected BeliefValueFunction offlineUpper;
    ...
    // / constructor
    public AndOrTree(Pomdp prob, ExpandHeuristic h, BeliefValueFunction L,
	...
	en.initChildren(problem.actions());
	// iterate through the AND nodes
	for (int action = 0; action < problem.actions(); action++) {
	    // type-cast, doon't yet know of a nicer way to do this
	    a = en.getChild(action);
	    // initialize this node, precompute Rba
	    a.init(action, en,  problem.getRewardFunction().eval(en.getBeliefState(), action,0)); //TODO: only infinite (stationary)
	    // pre-compute observation probabilities for the children of this
	    // node
	    pOba = problem.observationProbabilities(en.getBeliefState(), action,0); //TODO: only infinite (stationary)
	    ...
	    // }
	    a.initChildren(problem.observations(), pOba);
	    // iterate through new fringe OR nodes
	    for (int observation = 0; observation < problem.observations(); observation++) {
		// type-cast, doon't yet know of a nicer way to do this
		...
		o.init(problem.nextBeliefState(en.getBeliefState(), action,
			observation,0), observation, a); //TODO: only infinite (stationary)
	...
	return a.rba + problem.gamma() * Lba;
    }
	...
	return a.rba + problem.gamma() * Uba;
    }
	...
	//double Lba[] = new double[problem.actions()];
	ValueSelector vs = new ValueSelector(ValueSelector.MAX);
	for (HeuristicSearchAndNode a : root.getChildren()){
		vs.put(new Integer(a.getAct()), a.l);
	 //   Lba[a.getAct()] = a.l;
	}
	return ((Integer)vs.getRandom()).intValue();
    }
