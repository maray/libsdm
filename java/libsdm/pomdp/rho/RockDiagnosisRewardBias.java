...
public class RockDiagnosisRewardBias implements BeliefReward {
	...
	private int metals;
	SparseVector r;
	public RockDiagnosisRewardBias(int type,int rocks_configs,int states,int actions,double eta) {
		this(type,rocks_configs,states,actions,eta,0);
	}
	
	public RockDiagnosisRewardBias(int type,int rocks_configs,int states,int actions,double eta, int horizon) {
	...
	*/
	...
	private double bias_reward(int rc, int rocks, int metals) {
		...
		for (int r=0;r<rocks;r++)
			contrib+=RockDiagnosis.getRDConfigState(rc, r, metals) + 1;
